...
async def about(ctx):
	"""
	Outputs info about LoliBot, showing uptime, how to report issues, what settings where set in prefs.ini and credits.
	"""
	owner = bot.get_user(load_config.owner)
	em = discord.Embed(title="About Lolibot", colour=load_config.embedcolour, description=load_config.__description__)
	...
	em.set_footer(text="LoliBot is licensed under the MIT License")
...
if __name__ == "__main__":
	# Pre-Boot checks
	if not os.path.isfile("LoliBot/preferences.ini"):
		print(
	...
	if not os.path.isfile("LoliBot/settings/servers.json"):
		with open("LoliBot/settings/servers.json", "w") as fp:
			fp.write("{}")

	start_time = time.time()
	bot.load_extension("LoliBot.settings.settings")
	bot.load_extension("LoliBot.err_handle")
	bot.load_extension("LoliBot.logging")
